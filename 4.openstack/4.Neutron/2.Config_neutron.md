# Neutron agents
- Các plugin thông thường sẽ yêu cầu đối với các phần mềm cụ thể phải được chạy trên mỗi node để xử lý các gói dữ liệu. Nghĩa là, bất kỳ một node nào chạy nova-compute và những node chạy Openstack Networking service như: neutron-dhcp-agent, neutron-l3-agent, neutron-metering-agent or neutron-lbaasv2-agent.
...
- Ví dụ tên card mạng thật là eth2:
...
| type_drivers = local, flat, vlan, gre, vxlan, geneve | For example: type_drivers = flat,vlan,vxlan (xác định công nghệ, kiểu network sẽ sử dụng) |
