...
# 5. Lab
Server1: Controller + Cinder-scheduler + Cinder-volume (10.2.1.201)

Server2: Cinder-volume (10.2.1.201)

Backend: lvm

Cấu hình scheduler để khi tạo volume dưới 10gb sẽ tạo trên backend của server1, còn tạo từ 10gb trở lên sẽ tạo trên backend của server2

- Cấu hình server1, /etc/cinder/cinder.conf
```sh
[DEFAULT]
...
...
volume_backend_name = small
#filter_function = "volume.size < 10" (nếu dùng filter)
goodness_function = "(volume.size < 10) ? 100 : 20"
```

- Cấu hình server2, /etc/cinder/cinder.conf
```sh
[DEFAULT]
...
enabled_backends = lvm-large
...
target_protocol = iscsi
target_helper = tgtadm
volume_backend_name = large
#filter_function = "volume.size >= 10"
goodness_function = "(volume.size >= 10) ? 100 : 20"
```

- Sau đó restart dịch vụ trên cả 2 server

- Kiểm tra các dịch vụ cinder
```sh
root@hapu-lab-01:~# openstack volume service list
+------------------+-----------------------+------+---------+-------+----------------------------+
| Binary           | Host                  | Zone | Status  | State | Updated At                 |
+------------------+-----------------------+------+---------+-------+----------------------------+
| cinder-scheduler | hapu-lab-01           | nova | enabled | up    | 2026-01-17T02:58:18.000000 |
| cinder-volume    | hapu-lab-01@lvm       | nova | enabled | up    | 2026-01-17T02:58:23.000000 |
| cinder-volume    | hapu-lab-02@lvm-large | nova | enabled | up    | 2026-01-17T02:58:18.000000 |
+------------------+-----------------------+------+---------+-------+----------------------------+
```

- Khi này, tạo volume dưới 10gb sẽ tạo trên backend lvm của server1, tạo từ 10gb trở lên sẽ tạo trên backend lvm-large của server2
